...
| --- | --- | --- | --- |
| 1. 部门管理 | `/depts` | 5 | 全部实现 |
| 2. 员工管理 | `/emp` | 5 | 「查询全部（不分页）」待开发，见 4.6 |
| 3. 班级管理 | `/clazz` | 6 | 全部实现（含「查询所有」） |
| 4. 学员管理 | `/students` | 5 | 「违纪处理」无独立接口，通过修改学员实现，见 6.6 |
| 5. 数据统计 | `/report`、`/log` | 5 | 全部实现 |
| 6. 其他接口 | `/user`、`/upload` | 2 | 登录、文件上传 |
...
| 4.1 | 员工 | 分页查询员工列表 | POST | `/emp/getAllEmp` | 否 |
...
| 6.1 | 学员 | 分页查询学员列表 | GET | `/students/page` | 否 |
| 6.2 | 学员 | 新增学员 | POST | `/students` | 否 |
| 6.3 | 学员 | 根据 ID 查询学员 | GET | `/students/{id}` | 否 |
| 6.4 | 学员 | 修改学员 | PUT | `/students` | 否 |
| 6.5 | 学员 | 删除学员（批量） | DELETE | `/students/{ids}` | 否 |
...
- **免 Token**（拦截器放行）：`/user/login`、`/user/register`、`/emp/getAllEmp`、`/students/**`、Swagger 文档相关路径。
...
**备注说明**：名称唯一性由 Service 层校验，重复时返回 `code:0`。实体上虽有 `@NotBlank/@Size` 注解，但控制器的 `@RequestBody Dept` 未加 `@Valid`，因此长度校验目前不会在入口处生效，建议后续补上 `@Valid`。
...
**备注说明**：列表查询的 SQL 只查了姓名、性别、职位、头像、部门名、入职日期、修改时间这几列，因此 `id / username / phone / salary` 等字段在列表中恒为 `null`；如需完整信息请调用 4.4 详情接口。
...
**备注说明**：如需下拉选择场景，可临时调用 4.1 并传较大 `pageSize`；建议后续新增 `GET /emp/listAll` 之类的无分页接口。
...
**备注说明**：当前列表 SQL 未联查班主任姓名，`masterName` 恒为 `null`；`status` 由开课/结课时间与当前时间比较得出。
...
**备注说明**：`updateClazz` 控制器的 `@RequestBody ClazzDto` 未加 `@Valid`，字段校验由 Service 手动判空完成。
...
**备注说明**：该接口返回的是实体 `Student`（非 VO），`gender` 为数字编码、`isCollege` 为 `1/0`，与 6.1 列表（VO，中文/布尔）不一致，前端需注意。
...
**备注说明**：修改学员接口对 `violationCount`、`violationScore` 做了判空校验，缺失时返回 `code:0`。如未来需要「单次违纪登记 + 自动累加」，可再设计独立接口。
...
**备注说明**：当前 AOP 切面只填充了 `operateEmpId / costTime / returnValue` 三个字段，`className / methodName / methodParams / operateTime` 尚未填充（返回 `null`），属待完善项。此外，在 Token 放行路径（如 `/students/**`）上执行的操作无法取得当前操作人，`operateEmpId` 也会是 `null`。
